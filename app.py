...
# ── Page config ───────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="Pepper Disease Classifier",
    page_icon="🌿",
    layout="centered",
)

# ── Constants ─────────────────────────────────────────────────────────────────
MODEL_PATH       = Path(__file__).parent / "model" / "pepper_model.keras"
CLASS_STATS_PATH = Path(__file__).parent / "model" / "class_stats.npz"
# Alphabetical: Bacterial_Spot (index 0), Healthy (index 1) — raw_prob = P(Healthy)
...
def predict(model, pil_image: Image.Image):
    """raw_prob = P(Healthy) — Healthy is CLASS_NAMES[1] (alphabetically second)."""
...
# ── UI ────────────────────────────────────────────────────────────────────────
st.title("🌿 Pepper Leaf Disease Classifier")
st.caption(
    "Upload a bell pepper leaf image and the model will classify it as "
    "**Healthy** or **Bacterial Spot** using a MobileNetV2 transfer-learning model."
)
st.divider()
...
with st.sidebar:
    ...
    st.markdown(
        ...
        **Dataset:** [Plant Disease Detection](https://www.kaggle.com/datasets/karagwaanntreasure/plant-disease-detection) — Kaggle  
...
uploaded = st.file_uploader(
    "Upload a pepper leaf image (JPG / PNG)",
    type=["jpg", "jpeg", "png"],
    help="Clear, close-up images of the leaf surface produce the best results.",
)
...
if uploaded is not None:
    ...
    with col2:
        ...
        if rejected:
            ...
            color = "#2ecc71" if label == "Healthy" else "#e74c3c"
            st.markdown(
                f"""
            ...
            with st.expander("Raw probabilities"):
                healthy_prob   = raw_prob
                bacterial_prob = 1.0 - raw_prob
                st.write(f"**Healthy:** {healthy_prob * 100:.2f}%")
                st.progress(healthy_prob)
                st.write(f"**Bacterial Spot:** {bacterial_prob * 100:.2f}%")
                st.progress(bacterial_prob)

    st.divider()
    st.caption("This prediction is for educational purposes. Always verify with expert agronomic advice.")

else:
    st.info("👆 Upload a leaf image above to get a prediction.", icon="🖼️")
